...
# archives; abort on a mismatch. Skip only if the host has no sha256 tool.
if curl -fsSL "$base/cctop_checksums.txt" -o "$tmp/checksums.txt" 2>/dev/null; then
  if command -v sha256sum >/dev/null 2>&1; then
    ...
    got=""
    info "note: no sha256 tool found; skipping checksum verification"
  fi
  if [ -n "$got" ]; then
...
mkdir -p "$BIN_DIR"
install -m 0755 "$tmp/cctop" "$BIN_DIR/cctop"
